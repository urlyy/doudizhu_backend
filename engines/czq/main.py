...
from engines.czq.env.game import GameEnv
from engines.czq.evaluation.rlcard_agent import RLCardAgent
from engines.czq.evaluation.random_agent import RandomAgent
from engines.czq.evaluation.deep_agent import DeepAgent
...
def load_card_play_models():
    card_play_model_path_dict = {
        'landlord': os.path.join(os.getcwd(), "engines", "czq", "baselines", "douzero_ADP", "landlord.ckpt"),
        'landlord_up': os.path.join(os.getcwd(), "engines", "czq", "baselines", "sl", "landlord_up.ckpt"),
        'landlord_down': os.path.join(os.getcwd(), "engines", "czq", "baselines", "sl", "landlord_down.ckpt"), }
    players = {}
...
def create_env(room_id):
    ...
    print(env.game_infoset)


# 人只需要走，进行状态转移
# 注意如果传入空列表就是pass
def human_run(env, played_cards):
...
def ai_run(env):
    print("ai")
    print("身份", env.acting_player_position)
    print("选项",
          list(map(lambda cards: list(map(lambda c: EnvCard2RealCard[c], cards)), env.game_infoset.legal_actions)))
